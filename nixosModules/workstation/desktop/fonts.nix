...
let
  ...
  languageIsEnable = lang:
    config.workstation.system.i18n."${lang}".enable;

  languageIsDefault = lang:
    (languageIsEnable lang)
    &&
    config.workstation.system.i18n."${lang}".primary;
in
{
        ...
        mkMerge
          [
            ...
            (
              mkIf
                (
                  (languageIsEnable "chinese") ||
                  (languageIsEnable "japanese") ||
                  (languageIsEnable "korean")
                )
                {
                  environment = {
                    systemPackages = with pkgs;
            ...
            (
              mkIf
                (
                  (languageIsDefault "chinese") ||
                  (languageIsDefault "japanese") ||
                  (languageIsDefault "korean")
                )
                {
                  workstation = {
                    desktop = {
